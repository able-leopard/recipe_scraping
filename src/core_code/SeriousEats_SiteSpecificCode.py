...
"""
class SeriousEatsParser:
    
    domain_name = 'https://www.seriouseats.com'
    site_name = 'seriouseats.com'

    #manually adding all the page links to be fed to the to_do_list:
    base_link = 'https://www.seriouseats.com/recipes/topics/meal?page='

    start_to_do_links = []

    #manually iterating each page
    for i in range(464):
        current_link = base_link + str(i) + "#recipes"
    ...
    def find_sub_links(cls, soup_page) -> list:
        ...
        for i in soup_page.select('div.block-h-scroll__track'):
            i.decompose()

        #getting all the recipe links & next page links
        recipe_links = soup_page.select('div.module a.module__link')
    ...
    def get_recipe_name(cls, soup_page):
        result = []
        for i in soup_page.select('h1.recipe-title'):
    ...
    def get_serving_size(cls, soup_page):
        ...
        serving_size = soup_page.select('span.yield')
    ...
    def get_ingredients(cls, soup_page):
        ...
        result = []
        for i in soup_page.select('div.recipe-ingredients li.ingredient'):
...
#         original_ingredients =  result[0]
    ...
    def get_instructions(cls, soup_page):
        result = []
        for i in soup_page.select('div.recipe-procedures div.recipe-procedure-text'):
    ...
    def get_photo_link(cls, soup_page, link):
        ...
        photo = soup_page.select('div.se-pinit-image-container img')
        if photo:
