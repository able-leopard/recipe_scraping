...
class CookStrParser:
    ...
    domain_name = 'https://www.cookstr.com'
    site_name = 'cookstr.com'

    #manually adding all the page links to be fed to the to_do_list:
    base_link = 'https://www.cookstr.com/recipes/page/'

    start_to_do_links = []

    #there are 467 recipe pages
    for i in range(467):
        current_link = base_link + str(i)
        start_to_do_links.append(current_link)

    #taking out page 0 because it doesn't count
    ...
    def find_sub_links(cls, soup_page) -> list:
        ...
        recipe_links = soup_page.select('h4.articleHeadline a')
    ...
    def get_recipe_name(cls, soup_page):
        result = []
        for i in soup_page.select('h1.articleHeadline '):
    ...
    def get_serving_size(cls, soup_page):
        ...
        for i in soup_page.select('span.attrLabel'):
            i.decompose()

        serving_size = soup_page.select('p span')
    ...
    def get_ingredients(cls, soup_page):
        ...
        result = []
        for i in soup_page.select('div.recipeIngredients li'):
...
#         original_ingredients =  result[0]
    ...
    def get_instructions(cls, soup_page):
        result = []
        for i in soup_page.select('div.stepByStepInstructionsDiv li p'):
    ...
    def get_photo_link(cls, soup_page, link):
        ...
        photo = soup_page.select('div.mainImg img')
        if photo:
