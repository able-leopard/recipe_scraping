...
"""
class GeniusKitchenParser:
    
    domain_name = 'https://www.geniuskitchen.com'
    site_name = 'geniuskitchen.com'
    
    #manually adding all the page links to be fed to the to_do_list:
    base_link = "https://www.geniuskitchen.com/recipe?pn="
    ...
    #getting all the recipe page links
    @classmethod
    def find_sub_links(cls, soup_page) -> list:
    
        recipe_links = soup_page.select('h2 a')
    ...
    def get_serving_size(cls, soup_page):
        ...
        serving_size = soup_page.select('td.servings')[0].text
        
        original_serving_size = "\n"+serving_size.capitalize().replace("\n", " ") +"\n"
    ...
    def get_ingredients(cls, soup_page):
        ...
        result = []
        for i in soup_page.select('ul.ingredient-list li'):
            current_ingredient = i.text
            current_ingredient = re.sub(' +', ' ', current_ingredient) #getting rid of extra white spaces
...
#         original_ingredients =  result[0]
    ...
    def get_instructions(cls, soup_page):
        ...
        #getting rid of unnecessary tags
        for i in soup_page.select('div.directions-inner li div'):
            i.decompose()
        
        #getting the instruction tags
        result = []
        for i in soup_page.select('div.directions-inner li'):
