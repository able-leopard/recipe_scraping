...
"""
class ChowHoundParser:
    
    domain_name = 'https://www.chowhound.com'
    site_name = 'chowhound.com'
    
    #loading all these into the to_do_links to kick things off
    start_to_do_links = ['https://www.chowhound.com/recipes']
    start_viewed_links = []
    ...
    def find_sub_links(cls, soup_page) -> list:
        ...
        next_page_links = soup_page.select('div.nav-more a')

        #getting all the recipe links
        recipe_links = soup_page.select('div.fr_bxfoot a')
    ...
    def get_serving_size(cls, soup_page):
        ...
        serving_size = soup_page.select('span.frr_serves')
    ...
    def get_ingredients(cls, soup_page):
        ...
        result = []
        for i in soup_page.select('div.freyja_box h4, div.freyja_box ul li'):
...
#         original_ingredients =  result[0]
    ...
    def get_instructions(cls, soup_page):
        ...
        #getting rid of the number divider because its causing formatting problems
        for i in soup_page.select('span.number_divider'):
            i.decompose()
        
        #getting the instructions
        result = []
        for i in soup_page.select('div.frr_wrap strong, div.frr_wrap ol li, div.frr_wrap p strong'):
    ...
    def get_photo_link(cls, soup_page, link):
        photo = soup_page.select('img.lazy-loaded')
